"""
Modded nanogpt based on Keller Jordan's repo:

https://github.com/KellerJordan/modded-nanogpt

Differences from standard gpt2:
- RMS normalization replaces LayerNorm
- RoPE (rotary positional embedding) replaces standard position embedding
"""
import jax
...
from base import Array, PRNGKeyArray


class Rotary(eqx.Module):
    cos: jax.Array = eqx.field(static=True)
    sin: jax.Array = eqx.field(static=True)

    def __init__(self, dim: int, context_len: int, 
                 base: float = 1024.0, dtype=jnp.float32):
        assert dim % 4 == 0, f"dim={dim} is not divisible by 4 (half-truncated RoPE)"

        angular_freq = (1.0 / base) ** jnp.linspace(0, 1, dim//4, dtype=dtype)
        angular_freq = jnp.concatenate(
            [angular_freq, jnp.zeros(dim//4, dtype=dtype)],
            axis=0,
        )                                                   # [d/2]
        t = jnp.arange(context_len, dtype=dtype)            # [L]
        theta = jnp.einsum("i,j->ij", t, angular_freq)      # [L,d/2]
        self.cos = jnp.cos(theta)
        self.sin = jnp.sin(theta)

    @named_scope("gpt2.Rotary")
    def __call__(self, x: jax.Array) -> jax.Array:
        """x: [L, d]"""
        L, d = x.shape
        assert self.cos.shape[0] >= L
        assert d % 2 == 0
        x_fp32 = x.astype(jnp.float32)
        x1, x2 = jnp.split(x_fp32, 2, axis=-1)              # [L,d/2]
        cos = self.cos[:L]                                  # [L,d/2]
        sin = self.sin[:L]                                  # [L,d/2]
        y1 = x1 * cos + x2 * sin
        y2 = -x1 * sin + x2 * cos
        out = jnp.concatenate([y1, y2], axis=-1)            # [L,d]
...
class CausalSelfAttention(eqx.Module):
    ...
    attn_fc: nn.Linear
    linear: nn.Linear
    rotary: Rotary = eqx.field(static=True)
    rms: nn.RMSNorm = eqx.field(static=True)
    attn_dropout: nn.Dropout = eqx.field(static=True)
    linear_dropout: nn.Dropout = eqx.field(static=True)
    # mask: Array
    ...
    def __init__(self, config, *, key: PRNGKeyArray):
        ...
        self.rotary = Rotary(self.head_dim, config.context_length)
    ...
    def __call__(self, data, *, key: PRNGKeyArray):
        ...
        # Additional rotary from modded nanogpt
        # NOTE: normalize on the last axis (head dim)
        q, k = jax.vmap(jax.vmap(self.rms))(q), jax.vmap(jax.vmap(self.rms))(k)
        # NOTE: self.rotary is implemented to take data with size [L,d]
        q, k = jax.vmap(self.rotary)(q), jax.vmap(self.rotary)(k)
...
class TransformerBlock(eqx.Module):
    ...
    def __init__(self, config, *, key: PRNGKeyArray):
        ...
        self.rms = nn.RMSNorm(config.dim, use_weight=False, use_bias=False)
    ...
    def __call__(self, data, *, key: PRNGKeyArray):
        ...
        x = jax.vmap(self.rms)(data)                # NOTE: rmsnorm replaces layernorm
        x = self.attn(x, key=attn_key)              # [L,D]
        x += data                                   # [L,D]

        # y = jax.vmap(self.ln2)(x)                   # [L,D]
        y = jax.vmap(self.rms)(x)
        y = jax.vmap(self.expand_fc)(y)             # [L,D] -> [L,4D]
        # y = jax.nn.gelu(y)                          # [L,4D]
        y = jnp.square(jax.nn.relu(y))              # NOTE: this replaces gelu as suggested in modded-nanogpt
...
class ModdedNanogpt(eqx.Module):
